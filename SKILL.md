...
2. Call `AskUserQuestion` with:
   - Question: `"Which Claude model should the 15 audit agents use?"`
...
| `agent-13-bundle.md`  | `source.md` + `senior-auditor-sop.md` + `hacking-agents/signature-trust-agent.md` + `hacking-agents/shared-rules.md`                           |
| `agent-14-bundle.md`  | `source.md` + `senior-auditor-sop.md` + `hacking-agents/hook-ordering-agent.md` + `hacking-agents/shared-rules.md`                            |
| `agent-15-bundle.md`  | `source.md` + `senior-auditor-sop.md` + `hacking-agents/signature-gap-agent.md` + `hacking-agents/shared-rules.md`                            |
...
**Turn 3a — Spawn all 15 agents.** In one message, spawn all 15 agents as **parallel BACKGROUND Agent calls** (`run_in_background=true`). If Turn 1b set `{agent_model}`, pass `model={agent_model}` on every Agent call. If `{agent_model}` is unset (Turn 1b skipped — Codex, Gemini, others), omit the `model` parameter entirely — do NOT substitute any default. The orchestrator will receive a notification when each agent completes — do NOT poll or sleep. Single phase, no later spawns. Proceed to Turn 3b only after all 15 have notified completion.

Agents 1–9 use the **single-specialty prompt** (Turn 3a-i). Agents 10–12 use the **gap-hunter prompt** (Turn 3a-ii).

Local additions (not in upstream): agents **13** (signature-trust) and **14**
(hook-ordering) are single-specialty — use the Turn 3a-i prompt. Agent **15**
(signature-gap) is a gap hunter across signature-trust x access-control x
execution-trace — use the Turn 3a-ii prompt.
...
**Turn 3a-ii — Gap-hunter prompt template (agents 10–12 and 15, substitute real values):**
...
**Turn 3b — Wait for all 15 agents to complete.** Once every one of the 15 spawned agents has notified completion, proceed to Turn 4. Do NOT proceed to dedup until every agent has finished — let them run to natural completion. Do NOT poll or sleep; act only on completion notifications.
...
1. **Dedup.** Parse every FINDING and LEAD from the 15 agents. Group by `group_key` (Contract | function | bug-class). Exact-match first; merge synonymous bug_class within same (Contract, function). Keep best per group, number sequentially, annotate `[agents: N]`.
